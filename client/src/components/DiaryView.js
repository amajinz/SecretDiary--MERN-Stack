...
import { Button, ButtonGroup, Container, Collapse, Card, CardBody, CardTitle, CardText, Nav, Navbar, NavbarToggler } from 'reactstrap'
        ...
        {diary &&
          ...
          <Card>
            <Navbar light expand='sm' className='mt-2 mr-0'>
              <NavbarToggler className="view-function-toggle" onClick={this.toggle} />
              <Collapse isOpen={this.state.isOpen} navbar>
                <Nav className='ml-auto' navbar>
                  <ButtonGroup >
                    <Link to='#'><Button diaryID={params.id}>Edit</Button></Link>
                    <Link to='#'><DeleteDiaryModal diaryID={params.id} /></Link>
            ...
            <CardBody>
              <p>{new Date(diary.date).toDateString()} {new Date(diary.date).toLocaleTimeString()}</p>

              <CardTitle className='diary-title'>{diary.title}</CardTitle>
              <CardText>{diary.body}</CardText>
            </CardBody>
          </Card>
